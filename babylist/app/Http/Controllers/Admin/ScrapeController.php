...
class ScrapeController extends Controller
{
    public function show()
    {
        ...
        return view('admin.scrape-form', compact('shops', 'dreamBabyCategories', 'mimibabyCategories', 'hemaCategories'));
    }
...
//                foreach ($nodes as $node) {
    ...
    private function scrapemimibabyPageData($crawler) {
        ...
        $nodes = $crawler->filter('.card-body');
        $articles = array();
        $articles = $nodes->each(function($node){
//                foreach ($nodes as $node) {
            $article = new stdClass();
            ...
            $article->image = $node->filter('.product_info a .product_image .image img')->first()->attr('src');
            $article->price = $node->filter('.product-info .product-price-info .product-price-wrapper .product-price')->text();
            $article->desc = "";
            return $article;
        });
